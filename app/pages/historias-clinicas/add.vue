<script setup>
    ...
    // Estado del formulario basado en tu modelo historias_clinicas.js
    const form = reactive({
        nombre: '',
        apellidos: '',
        edad: null,
        ...
        enfermedades: '',
        antecedentes: '',
        pacienteId: '' // Se sugiere que esto venga de un buscador o parámetro
    })
    
    // Opciones del ENUM según tu modelo
    ...
    const handleRegistro = async () => {
      ...
      try {
        // Regla: Usar $fetch para ACCIONES (POST) dentro de try...catch
        ...
        // Redirigir a la lista de historias tras éxito
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <form @submit.prevent="handleRegistro" class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-2">
                <label class="text-xs font-bold text-gray-700 uppercase">Paciente ID (UUID)</label>
                <input v-model="form.pacienteId" required type="text" class="input-field font-mono text-sm" placeholder="ID del sistema" />
              </div>
